...
{	
	...
	{	
		...
		WgsPoint dest;
		if((dest = this.nav.latNav.Dest) != null)
		{
			this.DestinationLat.Text = dest.getLatitude().ToString("0.0000000");
			this.DestinationLon.Text = dest.getLongitude().ToString("0.0000000");
		}
		else
		{
			this.DestinationLat.Text = "N/A";
			this.DestinationLon.Text = "N/A";
		}
		this.HeaCurrKp.Text = this.nav.head.Kp.ToString("000");
		this.AltCurrKp.Text = this.nav.alt.Kp.ToString("000"); 
		this.LavNavCurrKp.Text = this.nav.latNav.Kp.ToString("000");
	}
	...
	protected void OnLatNavKpClicked (object sender, System.EventArgs e)
	{
		string[] query =  new string[]{"lnkp",this.QueryText.Text};
		
		double val;
		this.nav.ci.ExecuteQuery(query, out val);
		this.QueryOutputText.Text = val.ToString("0.00");
		this.LavNavCurrKp.Text = this.QueryText.Text;	
	}

	protected void OnAltKpClicked (object sender, System.EventArgs e)
	{
		string[] query =  new string[]{"alkp",this.QueryText.Text};
		
		double val;
		this.nav.ci.ExecuteQuery(query, out val);
		this.QueryOutputText.Text = val.ToString("0.00");
		this.AltCurrKp.Text = this.QueryText.Text;	
	}

	protected void OnHeaKpClicked (object sender, System.EventArgs e)
	{
		string[] query =  new string[]{"hekp",this.QueryText.Text};
		
		double val;
		this.nav.ci.ExecuteQuery(query, out val);
		this.QueryOutputText.Text = val.ToString("0.00");
		this.HeaCurrKp.Text = this.QueryText.Text;	
	}

	protected void OnDestinationClicked (object sender, System.EventArgs e)
	{
		string[] words = this.QueryText.Text.Split(new char[]{' ', ','}, StringSplitOptions.RemoveEmptyEntries);
		string[] query =  new string[]{"dest", words[0], words[1]};
		double val;
		this.nav.ci.ExecuteQuery(query, out val);
		this.QueryOutputText.Text = val.ToString("0.00");
		this.DestinationLat.Text = words[0];	
		this.DestinationLon.Text = words[1];
	}
}
