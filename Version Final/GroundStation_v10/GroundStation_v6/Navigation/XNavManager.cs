...
namespace GroundStation
{
	public class XNavManager
	{
		...
		{
            if (this.alt != null && (this.currMode == Mode.AUTONOMOUS || this.currMode == Mode.DIRECTED || this.currMode == Mode.ATTITUDE))
        ...
        public double GetSel(int n)
        {
            if (n == 0)
            {
                return this.alt.GetSel();
            }
            if (n == 1)
            {
                return this.head.GetSel();
            }
            if (n == 2)
            {
                return this.speed.GetSel();
            }
            else
            {
                return -999;
