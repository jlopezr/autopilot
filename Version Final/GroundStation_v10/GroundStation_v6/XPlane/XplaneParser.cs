...
namespace XPlane
{
	public class XplaneParser
	{
			...
			for (int i = 5; i < data.Length; i = i + 36)
			{
                if (p)
                {
                    ...
                    if (Angles != null && AoA != -999 && Speeds != -999 && Atmosphere != null && Position != null && ZuluTime != -999)//
                    {
                        ...
                        Atmosphere[1] = (float)((Atmosphere[1] + 273.15) * 1000);
		...
		private void UpdateAngles(byte[] data, int i)//
		{
			...
			float Yaw = (float)(BitConverter.ToSingle(data, i + 4 + 4 + 4 + 4)); //Magnetic Heading
            Angles[0] = Pitch;
            Angles[1] = Roll;
            Angles[3] = Yaw; //Magnetic Heading
			//Console.WriteLine ("Angles:{0},{1},{2}", Pitch, Roll, Yaw);
		}

        private void UpdateAnglesAoA(byte[] data, int i)//
        {
            float Hpath = (float)(BitConverter.ToSingle(data, i + 4 + 4 + 4)); //Horizontal Path
            Angles[2] = (float)(BitConverter.ToSingle(data, i + 4 + 4)); //Beta
            AoA = Hpath;
        ...
        public byte[] givemebytes(byte[] lastb)
        {
            ...
            while (tof)
            {
                ...
                if (sendbytes != null)
                {
                    if (sendbytes.Length > 47) //comprobar si el mensaje contiene la informacion de los tres sensores
                    {
                        if (lastb == sendbytes)
                        {
                            tof = true;
