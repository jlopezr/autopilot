...
// algorithm.
		...
		const double EQURAD = 6378137;
		const double iFLATTENING = 298.257223563;

		const double SGD_PI = 3.14159265358979323846;
		const double GEOD_INV_PI = SGD_PI;	
		
		static private double M0( double e2 ) {
			//double e4 = e2*e2;
		...
		/// <returns></returns>
        public static int geo_direct_wgs_84 ( double alt, double lat1,
		                        double lon1, double az1,
		                        double s, out double lat2, out double lon2,
		                        out double az2 )
		{
		    ...
		    double sinaz1 = Math.Sin(azm1), cosaz1 = Math.Cos(azm1);
			
			
		    if( Math.Abs(s) < 0.01 ) {	// distance < centimeter => congruency
			lat2 = lat1;
			lon2 = lon1;
			az2 = 180.0 + az1;
			if( az2 > 360.0 ) az2 -= 360.0;
			return 0;
		    } else if( cosphi1 != 0 ) {	// non-polar origin
			// u1 is reduced latitude
			double tanu1 = Math.Sqrt(1.0-e2)*sinphi1/cosphi1;
			double sig1 = Math.Atan2(tanu1,cosaz1);
			double cosu1 = 1.0/Math.Sqrt( 1.0 + tanu1*tanu1 ), sinu1 = tanu1*cosu1;
			...
			// AZIMUTH - FROM NORTH
			az2 = Math.Atan2(-sinaz,temp)/RADDEG;
			if ( Math.Abs(az2) < testv ) az2 = 0.0;
			...
			double paz = ( phi1 < 0.0 ? 180.0 : 0.0 );
		        double zero = 0.0f;
			return geo_direct_wgs_84( alt, zero, lon1, paz, dM, out lat2, out lon2, out az2 );
		    } 
		}
		...
		/// <param name="s"></param>
		/// <returns></returns>
        public static int geo_inverse_wgs_84( double alt, double lat1,
		                        double lon1, double lat2,
					double lon2, out double az1, out double az2,
		                        out double s )
		{
		    double a = EQURAD, rf = iFLATTENING;
		    int iter=0;
		    ...
		    if( (Math.Abs(lat1-lat2) < testv && 
			 ( Math.Abs(lon1-lon2) < testv) || Math.Abs(lat1-90.0) < testv ) )
		    {	
			...
			geo_inverse_wgs_84( alt, lat2,lon2,lat1,lon1, out az1, out az2, out s );
			b = az1; az1 = az2; az2 = b;
			return 0;
		    } else if( Math.Abs(cosphi2) < testv ) {
			// terminal point is polar
		        double _lon1 = lon1 + 180.0f;
			geo_inverse_wgs_84( alt, lat1, lon1, lat1, _lon1, 
						    out az1, out az2, out s );
			s /= 2.0;
			az2 = az1 + 180.0;
			if( az2 > 360.0 ) az2 -= 360.0; 
			return 0;
		    } else if( (Math.Abs( Math.Abs(lon1-lon2) - 180 ) < testv) && 
			       (Math.Abs(lat1+lat2) < testv) ) 
		    {
			// Geodesic passes through the pole (antipodal)
			double s1,s2;
			geo_inverse_wgs_84( alt, lat1,lon1, lat1,lon2, out az1, out az2, out s1 );
			geo_inverse_wgs_84( alt, lat2,lon2, lat1,lon2, out az1, out az2, out s2 );
			az2 = az1;
			s = s1 + s2;
			return 0;
		    } else {
			...
			do {
			    ...
			    if (Math.Abs(dlams) > GEOD_INV_PI && iter++ > 50) {
				return iter;
			    }
			...
			az2 = Math.Atan2(rnumer,denom)/RADDEG;
			if( Math.Abs(az2) < testv ) az2 = 0.0;
			if(az2 < 0.0) az2 += 360.0;
			...
			az1 = Math.Atan2(rnumer,denom)/RADDEG;
			if( Math.Abs(az1) < testv ) az1 = 0.0;
